...
(function() {
  var Fairy, Queue, exiting, fairy_id, get_server_ip, os, prefix, queue_names, redis, uuid,
    _this = this,
  ...
  Queue = (function() {
    ...
    Queue.prototype.regist = function(handler) {
      ...
      worker_id = uuid.v4();
      this.redis.hset(this.key('WORKERS'), worker_id, "" + (os.hostname()) + ":" + (get_server_ip()) + ":" + process.pid);
    ...
    Queue.prototype._try_exit = function() {
      queue_names.splice(queue_names.indexOf("" + this.fairy.id + ":" + this.name), 1);
      if (!queue_names.length) {
        process.exit();
      }
      return console.log("" + queue_names.length + " queues left: " + queue_names);
    };

    Queue.prototype._process = function(task) {
      var call_handler, errors, processing, requeue, retry_count, start_time,
        _this = this;
      if (exiting) {
      ...
      errors = [];
      this.handler_callback = function(err, res) {
      ...
      return (call_handler = function() {
        return _this.handler.apply(_this, __slice.call(task.slice(1, -1)).concat([_this.handler_callback]));
      })();
    };
